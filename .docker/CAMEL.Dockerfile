...
RUN apt-get install -y apache2 lib`client-dev build-essential libapache2-mod-wsgi-py3 --no-install-recommends
RUN a2enmod wsgi
RUN a2enmod rewrite
...
RUN apt-get update -y ; apt-get upgrade -y
RUN apt-get install -y python3-pip python3-dev --no-install-recommends
...
RUN pip install -r /var/www/CAMEL/requirements.txt
...
RUN rm -rf /var/www/CAMEL/requirements.txt
